...
plotWeather <- function(
  ...
  if( !is.na(startDate) ){
    ...
    mywtbl <- subset( mywtbl, date >= as.Date( startDate ) )
  }
  if( !is.na(endDate) ){
    tryCatch(
      as.Date(endDate)
    ...
    mywtbl <- subset( mywtbl, date <= as.Date( endDate ) )
  }
  ...
  if( plottype == "m" ){
    ...
    wvarSum <- dplyr::group_by(mywtbl,id,date)
  }
  ...
  if( plottype == "m" ){
    ...
    ggplot2::ggplot( data=wvarSum, aes( x = date, y = y, color=id ) ) +
      geom_line() +
      xlab('Date') +
